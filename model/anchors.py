...
class Anchors():
    ...
    def get_anchors(self, feature_map_size, anchor_range, anchor_size, rotations):
        ...
        # 每一个feature作为anchor中心位置，生成一系列的anchor
    ...
    '''
    names: 
    description: 根据模型推理得出的feature_map_size生成一系列的anchors
    param {*} self
    param {*} feature_map_size
    return {*}
    '''
...
'''
names: 
description: anchor分配
param {*} batched_anchors
param {*} batched_gt_bboxes
param {*} batched_gt_labels
param {*} assigners：每个cls的分配规则
param {*} nclasses
return {*}
'''
def anchor_target(batched_anchors, batched_gt_bboxes, batched_gt_labels, assigners, nclasses):
    ...
    for i in range(batch_size):
        ...
        for j in range(n_assigners): # multi anchors
            ...
            # 计算IOU2D并分配给最近的匹配对
            # [size of anchors * size of gt box]
            overlaps = iou2d_nearest(gt_bboxes, cur_anchors)  
        
            # max_overlaps是每一个gt与所有anchors最大的IOU
            max_overlaps, max_overlaps_idx = torch.max(overlaps, dim=0)
            
            # gt_max_overlaps指的是每一个anchor与所有gt最大的IOU
