...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        # 解析详见 (https://zhuanlan.zhihu.com/p/521277176)
...
class Neck(nn.Module):   
    def __init__(self, in_channels, upsample_strides, out_channels) -> None:
        super().__init__()
        ...
        for i in range(len(in_channels)):
            ...
            self.decoder_blocks.append(nn.Sequential(*decoder_block)) # 增加上采样层
...
class Head(nn.Module):
    def __init__(self, in_channel, n_anchors, n_classes) -> None:
        super().__init__()
...
class PointPillars(nn.Module):
    def __init__(self,
        ...
        self.neck = Neck(in_channels=[64, 128, 256], 
                         ...
                         out_channels=[128, 128, 128]) # 最后的输出是【128*3=384】
    ...
    '''
    names: 
    description: 编码输出左右的box
    param {*} self
    param {*} bbox_cls_pred
    param {*} bbox_pred
    param {*} bbox_dir_cls_pred
    param {*} batched_anchors
    return {*}
    '''
    ...
    '''
    names: 
    description: 定义pointpillars模型推理过程
    return {*}
    '''
